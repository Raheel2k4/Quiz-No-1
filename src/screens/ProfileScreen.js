...
export default function ProfileScreen({ navigation }) {
  // Assuming AppContext provides the user object and a logout function
  ...
  // Display user data
  const userName = user?.name || 'Instructor Name';
  const userEmail = user?.email || 'instructor@university.edu';
  
  // Settings items for the list
  const settingsItems = [
...
const styles = StyleSheet.create({
  ...
  container: { 
    ...
    paddingBottom: 40,
    paddingTop: 50, // Set to 50 for status bar clearance, matching Dashboard
    backgroundColor: '#f5f5f5' 
  },
  header: {
    flexDirection: 'row',
  ...
  backButton: {
    padding: 5,
    width: 40, // Ensure the title remains centered
  },
  ...
  settingItem: {
    ...
    marginBottom: 1, // Minimal margin to create distinct rows
    borderRadius: 10,
    borderBottomWidth: 0, // Removed individual border
    shadowColor: '#000',
  ...
  logoutButton: {
    flexDirection: 'row',
    backgroundColor: '#FF3B30', // Red for danger/exit action
    padding: 15, 
    borderRadius: 12, 
    alignItems: 'center', 
